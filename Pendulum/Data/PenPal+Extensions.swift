...
extension PenPal {
    ...
    func addEvent(id: UUID? = nil, ofType eventType: EventType, date: Date? = Date(), notes: String? = nil, pen: String? = nil, ink: String? = nil, paper: String? = nil, letterType: LetterType = .letter, ignore: Bool = false, noFurtherActions: Bool = false, trackingReference: String? = nil, withPhotos photos: [EventPhoto]? = nil, withCustomStationeryTypes customStationery: [CustomStationeryType]? = nil, in context: NSManagedObjectContext, recalculatePenPalEvent: Bool = true, saving: Bool = true) {
        ...
        if let customStationery {
            newEvent.updateStationery(to: customStationery, in: context, saving: false)
        }
    ...
    static func restore(_ data: [ExportedPenPal], to context: NSManagedObjectContext, usingArchive archiveDirectory: URL, overwritingExistingData: Bool = false, saving: Bool = true) -> ImportResult {
        ...
        for importItem in data {
            ...
            for event in importItem.events {
                ...
                let importedCustomStationery = (event.customStationery ?? []).map { CustomStationeryType(type: $0.type, icon: $0.icon, value: $0.value) }
                ...
                if let existingEvent = existingEvent {
                    ...
                    existingEvent.update(
                        ...
                        withPhotos: photos,
                        withCustomStationeryTypes: overwritingExistingData ? importedCustomStationery : nil,
                        in: context,
                    ...
                    penPal.addEvent(
                        ...
                        withPhotos: photos,
                        withCustomStationeryTypes: importedCustomStationery,
                        in: context,
                        recalculatePenPalEvent: false,
                        saving: false
                    )
